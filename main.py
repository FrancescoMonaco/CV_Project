...
assert len(sys.argv) == 2, "Usage: python FindBoxes.py <relative_path>"
data_path = sys.argv[1]
assert os.path.isdir(data_path)

# Create an HOG descriptor object
winSize = (64, 128)   # window size
blockSize = (32, 32)  # block size
blockStride = (8, 8)  # block stride
cellSize = (16, 16)   # cell size
nbins = 12            # num of bins
hog = cv2.HOGDescriptor(winSize,blockSize,blockStride,cellSize,nbins)

# Path to images
pos_im_path = data_path + r"/Positive"
neg_im_path= data_path + r"/Negative"
...
# Pad HOG feature vectors with zeros to make them consistent
padded_hog_features = []
for feature in data:
...
# Save the model:
joblib.dump(best_model, 'model_name.npy')
model = best_model
...
for file in images_paths:
 ...
 # Move through the rectangles
  ...
  if canny_h < 150 or canny_w < 150:
    detected_boxes = non_max_suppression(np.array(detected_boxes), probs=probabilities, overlapThresh=0.025)
  else:
